...
export function knowledgeContextToMarkdown(bundle: KnowledgeContextBundle): string {
  ...
  bundle.pages.forEach((page, index) => {
    ...
    lines.push(
      ...
      "",
      `- Path: \`${cell(page.path)}\``,
      `- Kind: ${page.kind}; concept type: ${cell(page.type ?? page.kind)}; distance: ${page.distance}; included words: ${page.includedWords}; source body words: ${page.wordCount}`,
      `- SHA-256: \`${page.contentSha256}\`${page.truncated ? "; content truncated at the budget boundary" : ""}`,
    );
    if (page.trust) {
      lines.push(
        `- Trust: ${page.trust.tier}; lifecycle: ${page.trust.status}; freshness: ${page.trust.stale ? "stale" : page.trust.staleAfter ? `fresh until ${page.trust.staleAfter}` : "not declared"}`,
        `- Generated by: ${page.trust.generated ? `${cell(page.trust.generated.by)}${page.trust.generated.at ? ` at ${cell(page.trust.generated.at)}` : ""}` : "not declared"}; verification events: ${page.trust.verified.length}`,
        `- Material sources: ${page.trust.sources.length}`,
      );
      page.trust.sources.forEach((source) => lines.push(`  - ${source.id ? `${cell(source.id)}: ` : ""}${cell(source.resource)}${source.author ? ` · ${cell(source.author)}` : ""}`));
    }
    if (page.attestedComputation) {
      lines.push(`- Attested Computation: runtime ${cell(page.attestedComputation.runtime ?? "missing")}; ${page.attestedComputation.parameters.length} declared parameter(s); contract is descriptive and was not executed`);
    }
    lines.push(
      "",
      `${fence}markdown`,
