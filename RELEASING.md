...
5. Run `pnpm release:check` and inspect its pack manifest: only the CLI build, Viewer build, canonical Skill, `NOTICE`, `LICENSE`, generated `THIRD_PARTY_LICENSES.txt`, and package metadata should ship. The check also validates the current version's frozen changelog and release notes.
...
For the first publication, verify that the unscoped `llm-wiki-canvas` name is still available, the real maintainer uses 2FA, and the account has a tested recovery path. Publish the exact uploaded tarball rather than rebuilding it. After the package exists, configure npm Trusted Publishing with a protected GitHub Environment and remove any bootstrap token. Normal defects should be handled by deprecation plus a fixed version; reserve unpublish for security or privacy incidents allowed by npm policy.
